...
import rbac
import hod_bundles   # Sept 2026: shared dept_sort_key so listings sort E01->E81
...
def atr_dashboard():
    ...
    rows.sort(key=lambda r: (
        ...
        hod_bundles.dept_sort_key(r["offering"].get("eff_dept")),  # then by HOD dept
        (r["offering"].get("dept_code") or "").split(",")[0],      #   then programme code
        (r["offering"]["course_code"] or ""),               # then by course
    ))

    # (Sept 2026) Order the REPORT listings BY HOD DEPARTMENT (E01 -> E81) so a
    # leader always finds a department's staff together. eff_dept is the faculty's
    # home department (who the HOD owns); dept_sort_key sends EXTERNAL/unset last.
    all_offerings.sort(key=lambda r: (
        hod_bundles.dept_sort_key(r["offering"].get("eff_dept")),
        (r["offering"].get("dept_code") or "").split(",")[0],
        r["offering"].get("year_of_study") or 0,
        (r["offering"].get("course_code") or ""),
    ))
    _ext_key = lambda b: ((b.get("dept_code") or "").split(",")[0],
                          (b.get("course_code") or ""))
    external_rows.sort(key=_ext_key)
    unassigned_rows.sort(key=_ext_key)
...
# fmt is 'pdf' only (Sept 2026: Excel report output removed).
# ----------------------------------------------------------------------------
...
def atr_report(cycle_code, offering_id, fmt):
    ...
    # render the PDF to a temp file and stream it. (Local imports keep the
...
# GET /atr/reports/<cycle_code>.pdf  —  ... or as ONE combined PDF (Sept 2026)
# ----------------------------------------------------------------------------
# The Dean's ask: let a leader pull EVERY report in their scope in one click,
# organised BY THE HOD who owns each teacher (the faculty's home department):
#     <Dept — Name>/<Staff Name>/<course report>.pdf     (VD/Dean: several depts)
#     <Staff Name>/<course report>.pdf                    (a HOD: one department)
# The .pdf twin is one document with a divider page starting each department.
#
# HOW IT WORKS (no new scoring, no new access rule):
#   * Scope: rbac.allowed_dept_codes(leader) — a HOD gets their own department(s);
#     the Vice-Dean/Dean (unrestricted) get every department, EXTERNAL and
#     UNASSIGNED included. Never a leak past a HOD's own department.
#   * Grouping + scoring: hod_bundles.build_department_groups scores each responded
#     delivery EXACTLY ONCE (electives already pooled) and buckets it by dept. This
#     replaced the old per-offering members_of() re-scan that made this route hang
#     at college scale — it is now O(n).
#   * Content: each report is rendered by the SAME build_pdf_report as the single
#     "PDF" button, so a file here is byte-for-byte the individual download.
#
# Both routes stay available even when ATR mode is OFF (a REPORT surface, not an
# ATR action) — the whole point of the Dean's "reports only" cycle. PDF-only.
# ----------------------------------------------------------------------------
...
def atr_reports_zip(cycle_code):
    ...
    import io, tempfile, scoring, report_export, hod_bundles
    from flask import send_file

    cy = _open_cycle_db(cyc)
    dl_weight = scoring.get_discussed_late_weight(master)
    # Group BY HOD (faculty home dept), scoring each responded delivery ONCE. Scope
    # follows the SAME rbac rule as the dashboard: a HOD gets their department(s),
    # the VD/Dean get everything. (This one-pass grouping is also the fix for the
    # old "download hangs forever" behaviour.)
    allowed = rbac.allowed_dept_codes(leader)
    groups = hod_bundles.build_department_groups(
        master, cy, cycle_code, allowed, dl_weight, bool(cyc["is_test"]))
    cy.close(); master.close()

    groups = [g for g in groups if g["results"]]
    if not groups:
    ...
    tmp.close()
    if len(groups) == 1:
        # A single department (a HOD) -> flat <Staff>/<report>.pdf.
        report_export.build_staff_foldered_pdf_zip(groups[0]["results"], tmp.name)
    else:
        # Several departments (Vice-Dean / Dean) -> nested
        # <Dept — Name>/<Staff>/<report>.pdf, one HOD folder per department.
    ...
    import activity_log
    activity_log.note(detail=f"{download_name} — {n} report(s), HOD/leader bulk download",
                      cycle_code=cycle_code, target_type="cycle")
    return send_file(io.BytesIO(data), mimetype="application/zip",
                     as_attachment=True, download_name=download_name)


@atr_bp.route("/atr/reports/<cycle_code>.pdf")
@leader_required
def atr_reports_pdf(cycle_code):
    leader = _current_leader()
    master = get_master()
    cyc = _cycle_by_code(master, cycle_code)
    if cyc is None:
    ...
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".pdf")
    tmp.close()
    report_export.build_grouped_combined_pdf(groups, tmp.name)   # divider per dept
    with open(tmp.name, "rb") as fh:
        data = fh.read()
    os.unlink(tmp.name)

    n = sum(len(g["results"]) for g in groups)
    download_name = f"AllStaffReports_{cycle_code}.pdf"
    import activity_log
    activity_log.note(detail=f"{download_name} — {n} report(s), HOD/leader combined PDF",
                      cycle_code=cycle_code, target_type="cycle")
